...
# fun any<A>(f :: (A -> Boolean), l :: List<A>) -> Boolean:
#   cases (List) l:
#     | empty => false
#     | link(x, r) => f(x) or any(f, r)
#   end
# end
...
data Exp:
  | val(v :: Val)  # val :: (Val -> Exp)
  | ident(s :: String) # ident :: (String -> Exp)
  | unexp(op :: Unop, e :: Exp) # unexp :: (Unop, Exp -> Exp)
...
fun as-list<A>(s :: Sexp) -> Result<List<Sexp>>:
  cases (Sexp) s:
    | s-list(l) => ok(l)
    | else => err(ParseError("expected list, got " + to-repr(s)))
  end
  ...
  cases (Sexp) s:
    | s-list(l) =>
      if eq(l.length(), 2):
        ...
        # unexp :: (Unop, Exp -> Exp)
        # lift2(unexp) :: (Result<Unop>, Result<Exp> -> Result<Exp)
        
        # cases (Result) parseUnop(l.get(0)):
        #   | ok(u) =>
        #     cases (Result) parseExp(l.get(1)):
        #       | ok(e) =>
        #         ok(unexp(u, e))
        #       | err(e) => err(e)
        #     end
        #   | err(e) => err(e)
        # end
        # seq(parseUnop(l.get(0)), lam(u):
        #     seq(parseExp(l.get(1)), lam(e):
        #         ok(unexp(u, e))
        #       end)
        #   end)
      else:
